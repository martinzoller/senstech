...
def get_next_project_id(customer_id):
    ...
    latest_from_item = frappe.db.sql("""SELECT SUBSTR(`item_code`,8,2) AS project_no FROM `tabItem` WHERE SUBSTR(`item_code`,3,5) = '-{cust}-' GROUP BY `project_no` ORDER BY `project_no` DESC LIMIT 1""".format(cust=short_customer_id), as_list=True)
