frappe.ui.form.on('Item', {
	...
	validate(frm) {
	    ...
	    if (cur_frm.doc.is_sales_item&&cur_frm.doc.is_purchase_item) {
			validation_error(frm, 'is_sales_item', __("Ein Artikel darf nicht als 'Einkaufs-' wie auch 'Verkausartikel' definiert sein."));
	    }
	    ...
	    if (['Serieprodukte OEM','Eigenprodukte'].includes(frm.doc.item_group)  && !cur_frm.doc.has_batch_no) {
	        validation_error(frm, 'has_batch_no', __("Serie- und Eigenprodukte müssen eine Chargennummer haben"));
	    }
		...
		if (frm.doc.item_group) {
    		frappe.db.get_doc("Item Group", frm.doc.item_group).then( grp => {
				if (frm.doc.is_purchase_item && grp.parent_item_group != 'Einkauf') {
					validation_error(frm, 'item_group', __("Die Artikelgruppe '"+frm.doc.item_group+"' ist nicht für Einkaufsartikel vorgesehen. Bitte eine andere Gruppe auswählen."));
				}
				if (frm.doc.is_sales_item && grp.parent_item_group != 'Verkauf') {
					validation_error(frm, 'item_group', __("Die Artikelgruppe '"+frm.doc.item_group+"' ist nicht für Verkaufsartikel vorgesehen. Bitte eine andere Gruppe auswählen."));
				}
    		});
		}
	},
